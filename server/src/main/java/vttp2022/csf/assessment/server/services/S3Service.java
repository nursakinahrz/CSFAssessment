...
public class S3Service {
    ...
    public String getMap(MultipartFile file) throws IOException{
        ...
        String key = UUID.randomUUID().toString()
            .substring(0, 8);
        System.out.println("maps/%s".formatted(key));
        System.out.println(s3Client.doesBucketExistV2("nrzbucket"));
        System.out.println(s3Client.doesBucketExistV2("nrzbucket"));
        ...
        System.out.println("maps/%s.%s".formatted(key, filenameExt));
        if(filenameExt.equals("blob"))
            filenameExt = filenameExt + ".png";
        PutObjectRequest putRequest = 
            new PutObjectRequest(
                "bucketnrz", 
                "maps/%s.%s".formatted(key, filenameExt), 
                file.getInputStream(), 
                metadata);
        putRequest.withCannedAcl(
                CannedAccessControlList.PublicRead);
        s3Client.putObject(putRequest);
        return "maps/%s.%s".formatted(key, filenameExt);
    }
}
